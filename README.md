...
| `dot_config/waybar/` | Waybar status bar config and styles (imports palette from `colors.css`) |
| `dot_config/rofi/` | Rofi-wayland launcher config (imports palette from `colors.rasi`) |
| `dot_config/swaync/` | SwayNotificationCenter config and styles (imports palette from `colors.css`) |
| `dot_config/ghostty/` | Ghostty terminal config — uses built-in `GruvboxMaterial` theme |
| `dot_config/flavours/` | flavours config, Gruvbox Material base16 scheme, and per-app mustache templates |
...
A full keyboard-driven Wayland desktop, handcrafted — no ML4W, HyDE, or other opinionated frameworks. The Gruvbox Material colour palette is managed by [flavours](https://github.com/Misterio77/flavours) (a base16 theme manager), so all apps stay in sync and the theme can be switched with a single command.
...
8. **Colour scheme** — `flavours apply gruvbox-material` generates palette files consumed by Waybar, Rofi, swaync, and Hyprland
9. **Language runtimes** — mise installs Node, Erlang, Elixir, and Python
...
| Component | Package | Role |
|---|---|---|
| **Status bar** | `waybar` | Top bar showing workspaces, active window title, clock, battery, volume, network, and notifications. Driven by a JSON module config and a CSS stylesheet. Colours are imported from `~/.config/waybar/colors.css`, generated by flavours. Config: `~/.config/waybar/`. |
| **App launcher** | `rofi-wayland` | Keyboard-driven launcher for apps (`Super+Space`), commands (`Super+Shift+Space`), open windows, emoji, and clipboard history. Colour variables are imported from `~/.config/rofi/colors.rasi`, generated by flavours. Config: `~/.config/rofi/config.rasi`. |
...
| Component | Package | Role |
|---|---|---|
| **Terminal** | `ghostty` | Primary terminal emulator. GPU-accelerated, GTK4-native, zero-config on Linux. Configured with JetBrains Mono Nerd Font and Ghostty's built-in `GruvboxMaterial` theme. `gtk-titlebar = false` tells it to let Hyprland draw the window border. Config: `~/.config/ghostty/config`. |
...
| Component | Package | Role |
|---|---|---|
| **Colour scheme** | `flavours` (AUR) | base16 theme manager. Reads `~/.config/flavours/config.toml` and generates palette files for Waybar, Rofi, swaync, and Hyprland from mustache templates. Switch theme with `flavours apply <scheme>`. The Gruvbox Material scheme and custom templates are bundled in this repo under `dot_config/flavours/`. |
